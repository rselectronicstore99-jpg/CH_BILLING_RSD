...
if not st.session_state.is_logged_in:
    ...
    if show_login_form:
        ...
        with st.form("shop_registration_form"):
            ...
            with col1:
                lic_1 = st.text_input("Lic No 1 *").upper().strip()
            with col2:
                lic_2 = st.text_input("Lic No 2 - Optional").upper().strip()
                    ...
                    try:
                        with st.spinner("Creating account in database..."):
                            ...
                            if success:
                                ...
                                st.error("Data not saved. Check your network connection or credentials.")
                    except Exception as e:
                        st.error(f"Error: {e}")
        st.stop()
